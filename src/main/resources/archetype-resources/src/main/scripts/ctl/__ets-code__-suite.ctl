...
<ctl:package xmlns:ctl="http://www.occamlab.com/ctl"
  ...
  xmlns:tns="http://www.opengis.net/cite/${ets-code}"
  xmlns:saxon="http://saxon.sf.net/"
  xmlns:tec="java:com.occamlab.te.TECore"
  xmlns:tng="java:${package}.TestNGController">
  ...
  <ctl:function name="tns:run-ets-${ets-code}">
    ...
    <ctl:param name="outputDir">The directory in which the test results will be written.</ctl:param>
		...
		<ctl:description>Runs the ${ets-code} ${symbol_dollar}{version} test suite.</ctl:description>
    <ctl:code>
      <xsl:variable name="controller" select="tng:new(${symbol_dollar}outputDir)" />
      <xsl:copy-of select="tng:doTestRun(${symbol_dollar}controller, ${symbol_dollar}testRunArgs)" />
    </ctl:code>
	</ctl:function>

   <ctl:suite name="tns:ets-${ets-code}-${symbol_dollar}{version}">
     <ctl:title>${ets-title-short} Conformance Test Suite</ctl:title>
	  ...
	  <ctl:code>
        <xsl:variable name="form-data">
           <ctl:form method="POST" width="800" height="600" xmlns="http://www.w3.org/1999/xhtml">
             <h2>${ets-title-short} Conformance Test Suite</h2>
             <div style="background:${symbol_pound}F0F8FF" bgcolor="${symbol_pound}F0F8FF">
             ...
             <fieldset style="background:${symbol_pound}ccffff">
               <legend style="font-family: sans-serif; color: ${symbol_pound}000099; 
			                 background-color:${symbol_pound}F0F8FF; border-style: solid; 
        ...
        <xsl:variable name="iut-file" select="$form-data//value[@key='doc']/ctl:file-entry/@full-path" />
          ...
          <entry key="ics"><xsl:value-of select="${symbol_dollar}form-data/values/value[@key='level']"/></entry>
		    </properties>
		   </xsl:variable>
       <xsl:variable name="testRunDir">
         <xsl:value-of select="tec:getTestRunDirectory(${symbol_dollar}te:core)"/>
       </xsl:variable>
       <xsl:variable name="test-results">
        <ctl:call-function name="tns:run-ets-${ets-code}">
			    <ctl:with-param name="testRunArgs" select="${symbol_dollar}test-run-props"/>
          <ctl:with-param name="outputDir" select="${symbol_dollar}testRunDir" />
			  </ctl:call-function>
		  </xsl:variable>
      <xsl:call-template name="tns:testng-report">
        <xsl:with-param name="results" select="${symbol_dollar}test-results" />
        <xsl:with-param name="outputDir" select="${symbol_dollar}testRunDir" />
      </xsl:call-template>
...
select="concat(substring-after(${symbol_dollar}testRunDir, 'users/'), '/html/')" /> directory.
        </ctl:message>
        <xsl:if test="xs:integer(${symbol_dollar}test-results/testng-results/@failed) gt 0">
          <xsl:for-each select="${symbol_dollar}test-results//test-method[@status='FAIL' and not(@is-config='true')]">
            <ctl:message>
Test method <xsl:value-of select="./@name"/>: <xsl:value-of select=".//message"/>
        ...
        <xsl:if test="xs:integer(${symbol_dollar}test-results/testng-results/@skipped) eq xs:integer(${symbol_dollar}test-results/testng-results/@total)">
        <ctl:message>All tests were skipped. One or more preconditions were not satisfied.</ctl:message>
        <xsl:for-each select="${symbol_dollar}test-results//test-method[@status='FAIL' and @is-config='true']">
          <ctl:message>
  ...
  <xsl:template name="tns:testng-report">
    ...
    <xsl:variable name="stylesheet" select="tec:findXMLResource(${symbol_dollar}te:core, '/testng-report.xsl')" />
    <xsl:variable name="reporter" select="saxon:compile-stylesheet(${symbol_dollar}stylesheet)" />
    <xsl:variable name="report-params" as="node()*">
      <xsl:element name="testNgXslt.outputDir">
        <xsl:value-of select="concat(${symbol_dollar}outputDir, '/html')" />
      </xsl:element>
    </xsl:variable>
    <xsl:copy-of select="saxon:transform(${symbol_dollar}reporter, ${symbol_dollar}results, ${symbol_dollar}report-params)" />
  </xsl:template>
</ctl:package>
